...
public class Itemset implements Comparable<Itemset> {
    ...
    /**
     * Compares itemsets primarily by expected utility (DESCENDING - higher is better),
     * then by existential probability (descending), then by size (ascending).
     * This provides a natural ordering for top-k mining where HIGHER utility comes FIRST.
     * 
     * @param other The other itemset to compare
     * @return Negative if this > other (this should come first), positive if this < other
     */
    @Override
    public int compareTo(Itemset other) {
        // Primary: Expected utility (DESCENDING - reverse comparison)
        // We want HIGHER utility to be "less than" so it comes first in sorted order
