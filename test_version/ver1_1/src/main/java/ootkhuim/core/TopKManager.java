...
public class TopKManager {
    ...
    public TopKManager(int k) {
        ...
        this.k = k;
        // TreeSet with natural ordering - Itemset.compareTo() returns descending order
        // So first() will be highest utility, last() will be lowest
    ...
    private void updateThreshold() {
        if (topKSet.size() >= k) {
            ...
            if (kthItemset != null) {
                threshold.set(kthItemset.getExpectedUtility());
            }
        } else {
            threshold.set(0.0);
        }
    ...
    public boolean validateConsistency() {
        ...
        if (topKSet.size() >= k) {
            ...
            if (Math.abs(kthItemset.getExpectedUtility() - threshold.get()) > EPSILON) {
                return false;
            }
        }
