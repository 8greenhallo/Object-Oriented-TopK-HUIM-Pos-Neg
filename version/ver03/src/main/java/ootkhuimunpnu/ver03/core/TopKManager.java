...
/**
 * Top-K manager for maintaining the best k itemsets
 * 
 * Design Principles:
 * - Encapsulation: Internal data structures hidden from clients
 * - Efficiency: O(log k) insertion using TreeSet
 * - Thread-Safety: Sequential version doesn't need synchronization
 * - Dynamic Threshold: Threshold updates as better itemsets are found
 * 
 * Key Operations:
 * - tryAdd: Attempts to add an itemset to top-K
 * - getThreshold: Returns current k-th highest utility
 * - getTopK: Returns final top-K results
 * 
 * MODIFICATION:
 * - Add Quick check before check new itemset in tryAdd function.
 * 
 * @author Meg
 * @version 3.0
 */
public class TopKManager {
    private final int k;
    private final TreeSet<Itemset> topKSet;                 // TreeSet maintains sorted order (highest utility first)
    private final Map<Set<Integer>, Itemset> itemsetMap;    // HashMap for O(1) duplicate checking
    ...
    public synchronized boolean tryAdd(Set<Integer> items, double eu, double ep) {
        ...
        if (eu < threshold - Configuration.EPSILON && topKSet.size() >= k) {
            return false;
        }
        ...
        if (existing != null) {
            ...
            if (existing.getExpectedUtility() < eu - Configuration.EPSILON) {
