...
/**
 * Main mining engine for OOTK-HUIM-U± algorithm
 * 
 * @author Meg
 * @version 2.2
 */
public class MiningEngine {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        System.out.println("=== OOTK-HUIM-U± ===");
    ...
    private Map<Integer, UtilityList> twoPassInitialization(List<Transaction> rawDatabase) {
        ...
        Map<Integer, UtilityList> orderedUtilityList = buildUtilityLists(rawDatabase, itemProb);

        // Clear temporary structures to free memory
        itemRTWU.clear();
        itemProb.clear();

        System.out.println("Single item utility-lists created: " + orderedUtilityList.size());
